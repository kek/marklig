...
| **C. Export & print** | PDF, self-contained HTML, print pipeline | ✅ Shipped 2026-05-08 (PDF via OS print dialog; native macOS print-to-PDF added 2026-07-28, issue #55) |
...
**Print.** `Cmd/Ctrl + P` renders the export HTML into a hidden same-process iframe, then calls `contentWindow.print()`. The same-document path is used (rather than `window.open`) because Tauri's webview doesn't reliably forward print events from popups. The export stylesheet's `@media print` block drops page padding, forces black-on-white, and adds `page-break-inside: avoid` for code/blockquotes.

**Export as PDF (native).** File → Export → PDF… renders the same self-contained export HTML to a PDF without the OS print dialog (issue #55). The JS side prompts for a destination and invokes the Rust `export_pdf` command (`src-tauri/src/commands/export_pdf.rs`) with the HTML + path. On macOS the command drives `WKWebView.createPDF(configuration:completionHandler:)` on an offscreen webview: it hops to the main thread (WKWebView is main-thread-only), loads the HTML, and on the navigation-delegate's `didFinishNavigation` captures the PDF, blocking the command's worker thread on a channel until the async completion handler reports back. Windows (WebView2 `PrintToPdf`) and Linux (WebKitGTK print operation) return `Unsupported` for now and continue to use the OS print dialog's "Save as PDF". The webview-capture path can't be exercised in headless CI (no display); the pure destination-validation and file-write helpers are unit-tested, and the JS save-dialog wiring is covered in `tests/shell/export-pdf.test.ts`.
...
**Shipped:**
- **File associations.** macOS `CFBundleDocumentTypes` + `UTExportedTypeDeclarations`, Windows registry entries, Linux `.desktop` MimeType for `.md` / `.markdown` / `.mdx` / `.mdown`. macOS also registers with LaunchServices (PR #9) so Finder routing works without re-opening the app. Files arrive via `RunEvent::Opened`; Rust buffers them until the session has restored, then routes them through the same registry-aware router as any other launch argument — there is no frontend event or fallback dialog in this path any more (see the session-ownership bullet below). Finder-launched files prefer a window already showing the file's tree, otherwise spawn a new one (PR #28).
...
- **Session restore and open-routing are now owned by Rust** (`src-tauri/src/session/`, 2026-08-11). There is no privileged `main` window and no declarative window in `tauri.conf.json` — Rust plans the restore, creates every window at `RunEvent::Ready`, and routes every `md <dir>` / `md <file>` launch argument against the live window registry, so cold-start arguments see the windows the session just restored instead of racing them. See CLAUDE.md's "Session, launch, and open-routing" section for the module breakdown.
...
- [x] **Step 4 — Crypto core (`marklig-sync-core`).** Workspace crate with Noise XK pairing, ChaCha20-Poly1305 envelope, sync op log + Lamport clock. 25 unit tests; builds for Android via Tauri. Consumed by `pairing.rs`, `pairing_ws.rs`, `sync_log.rs`, `sync_watcher.rs`, `commands/mobile_pairing.rs`, `commands/mobile_sync.rs`. Plan: `docs/superpowers/plans/2026-05-17-mobile-companion-4-crypto-core.md`. PR #_.
- [x] **Step 5 — Desktop pairing UX.** QR modal + settings → Pairings pane (`src/ui/pairings-pane.ts`, mounted from `src/ui/preferences.ts`); QR rendered as inline SVG from the `marklig-pair://v2/` payload. **Deviation:** folder sync is an "Add folder" picker inside the Pairings pane, not the per-folder "Sync this folder to phone" menu action the plan described.
- [x] **Step 6 — LAN transport.** Shipped as a **WebSocket** server on a fixed port (`src-tauri/src/pairing_ws.rs`, `WS_PORT` 14200), *not* the mDNS + raw TCP + WebRTC stack this line originally described. The pivot let the phone side drive pairing from pure JS instead of a hand-written Kotlin plugin; the rationale and trade-offs are in that module's header. Consequences: no WebRTC fallback (LAN only), and the phone dialled a frozen address until the mDNS work below.
- [x] **Step 6b — mDNS discovery (`_marklig-sync._tcp`), desktop half.** `src-tauri/src/mdns.rs` announces instance name + `WS_PORT` (TXT `proto=ws`) while the pairing server is armed, and exposes `mdns_browse_peers` / `mdns_resolve_instance` so a peer can resolve the desktop by name instead of a stored IP. Proven by `src-tauri/tests/mdns_discovery.rs` — real multicast between two daemons, with negative guards. **Phone half not done:** `src/shell/mobile-sync-client.ts` still dials `pairing.last_host`, so a DHCP change still breaks reconnect until the Android side resolves the name. That needs a device to verify.
- [x] **Step 7 — Phone pairing UX + sync wire-up.** Scan/paste QR, run the handshake, subscribe to a folder, render synced content (`src/ui/mobile-pair-form.ts`, `src/ui/mobile-synced-view.ts`, `src/shell/mobile-sync-client.ts`). **Deviation:** connects to the QR's `host` address; "resolve mDNS" is still outstanding per Step 6b.
