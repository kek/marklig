...
- **The Typst preview recompiles when an imported file changes.** Editing a `.typ` recompiled the preview on every keystroke, but editing a file it `#import`s did nothing — the pane kept rendering the old imports until the entry file was touched. That is the normal shape of a real Typst document rather than an edge case: `Karl Eklund.typ` in Karl's CV repo is nine lines importing `template.typ` and `strings.typ`, which reach `contact.typ`, `intro.typ`, `work.typ` and `education.typ`, so essentially all of the editing happens in files the preview did not watch. The compiler half already worked and needed no change: `ViewerWorld` caches no file slots, so `World::source` re-reads from disk on every call and comemo revalidates against the new content — which is also why there is no `reset()` here of the kind typst-cli needs for its slot cache. Only the *trigger* was missing, and with it any record of which files a compile had read. The world now collects that itself (`begin_dependency_capture` / `finish_dependency_capture`, surfaced as `CompileResult.dependencies`) by recording what the compiler resolved rather than by parsing import statements — the same approach `typst watch` takes, and the reason a file the entry document never names is still found. That distinction is the feature: the CV's `education.typ` is three levels down and appears in no import statement the entry file contains. Package files are excluded, being immutable in the shared cache. Watching is a **separate** watcher from the document one (`commands/typst_dep_watcher.rs`, `src/shell/typst-dep-watcher.ts`) because the two answer different questions: an external change to the *open file* means the buffer on screen is stale and the reconcile flow must choose between reloading, prompting, and declaring the file orphaned, whereas a changed *import* leaves the buffer correct and only the render stale — so the response is to recompile and pointedly not to touch the editor. Routing imports through `installWatcher` would have inherited reload-and-prompt behaviour that is wrong for every one of them. Like the document watcher it watches directories non-recursively (one per distinct parent, deduplicated — imports commonly sit in a subdirectory) rather than individual files, because most editors save atomically by replacing the file. A compile that *fails* may only add to the watch set, never replace it: a parse error in the entry file dies before resolving any import, so adopting that compile's near-empty set would unwatch every import, and the fix the user then makes in one of them — the overwhelmingly likely next edit — would not retrigger. A later successful compile is still free to shrink the set, so deleting an import does stop watching it. The set is re-synced after every compile but reaches the backend only when it actually differs, so typing does not tear down and re-arm kernel watches every 300 ms. The frontend filters events against its own set for the same `EventTarget::Any` reason as #145, which also gives the wanted behaviour when two windows are open on one project: both recompile. Covered at each layer, since no single layer can show the feature works — Rust integration tests that a three-level import chain is captured and that a changed leaf reaches the next render, a test that a failed compile does not shrink the set, a real-filesystem test that an import in a *subdirectory* is delivered while a non-dependency sibling in that same directory is not (the document watcher only ever watches one directory, so nothing in the tree covered the multi-directory case), and vitest coverage of set-diffing, teardown, and the cross-window filter.
...
- **Typst dependency watches recover when an import directory is created.** Missing parents are watched through their nearest existing ancestor, including nested directory creation, so creating the import refreshes the preview without editing the entry file. Failed watch installations now preserve the previous working set and remain retryable instead of being cached as successful.

- **Indented code blocks now render as code.** CommonMark's other code form — a block indented four spaces rather than wrapped in a fence — got no decoration at all, in either mode: `codeblocksProducer` matched `fence` and markdown-it emits these as `code_block`. The construct is not exotic; it is the ordinary way prose embeds a snippet under a list item, where the snippet is indented past the item's content anyway and a fence would be noise. Such a block came out as plain paragraph text in the monospace-free body font, with reading mode's paragraph reflow and inline-marker passes running straight over it — so a `*p` in a C snippet or a backtick in a shell one was read as emphasis and elided, quietly altering the code on screen. Both halves are fixed. Every line in the token's map now carries `cm-md-code-body` (no Shiki pass: an indented block has no info string, and guessing a grammar would colour the wrong one), and the block is registered in reading mode's `codeRanges` so the marker passes leave its contents alone. Reading mode also elides the block's leading indentation, on the same principle that hides a fence's backticks — the four-or-more spaces are this construct's marker. It elides the indent *common* to the block's non-blank lines, never each line's own, so the snippet's internal nesting survives; blank interior lines are skipped when computing that prefix, or one would drag it to zero and dedent nothing. The elision is an inline replace beginning exactly where the body line's `Decoration.line` sits, which is the boundary that has dropped line decorations before (see the `ELIDE_FENCE` comment), so a test builds the real merged reading-mode field and asserts both survive together rather than trusting the producers in isolation.

- **Mission Control no longer labels every window "Märklig"** (#159, reopened). The OS window title — what Mission Control, long-pressing the dock icon, and the Window menu read — was still the string each window was *created* with, so three windows open on three different projects were three identical labels and the multi-window workflow had no way to tell them apart. #159 was closed as fixed, and the part it fixed was real: `buildWindowTitle` learned to fall back to the folder basename, and the call sites were plumbed with `currentFolder`. None of it ever reached the OS. `getCurrentWindow().setTitle()` is a permissioned IPC call, and `core:window:allow-set-title` was not in `src-tauri/capabilities/default.json` — `core:window:default` bundles two dozen permissions but only the *getter* `allow-title`, so the setter has to be listed explicitly. Every call was therefore rejected by the ACL, and `setWindowTitle` caught the rejection in the same `catch` that serves "there is no Tauri API here" (vite-only `npm run dev`, unit tests) and assigned `document.title` instead. That fallback is correct in the case it was written for, which is why nothing looked wrong: on macOS the drawn title is hidden (`hiddenTitle: true`) and the name the user actually sees comes from the custom titlebar DOM via `toolbar.setPath` — a completely separate code path that kept working. So the visible chrome said `README.md — nerves_anbernic` while the OS said `Märklig`, and the only surfaces that disagreed were ones you have to leave the app to look at. The capability is now granted, and the failure is no longer silent: a rejected call warns and names the permission, because a title that fails to apply has no on-screen symptom and would otherwise regress the same way. Independently of the frontend, Rust now titles each window correctly *at creation* (`window_title` in `src-tauri/src/session/mod.rs`, replacing a hardcoded `.title("Märklig")`) rather than leaving a placeholder for the webview to correct later — it already holds the folder and path it is restoring, the title is right on the first frame instead of after boot, and Mission Control reads it during the window's whole pre-boot life. Two frontend call sites also dropped the project name where the window still had a project open: closing the last document in a folder (`applyProjectFallbackBuffer`) and having the open file deleted externally (the watcher's `onRemoved`) both passed no folder, retitling a window that still shows a project tree to the bare app name. Both now pass it. A blank window's fallback title was the literal `"Viewer"`, which is not what the app is called; it is now the product name, matching Rust and `tauri.conf.json`. Tested on both sides — eight Rust cases for the creation-time title (file + project, either alone, neither, the file-*is*-the-folder case, a dirty crash-restore, and basenames with trailing or Windows separators) and a new `tests/ui/titlebar-os-title.test.ts` that runs against a *present* `@tauri-apps/api/window` so it can assert what is pushed to `setTitle` and that a rejection warns; `titlebar.test.ts` keeps covering the API-absent fallback, which is why the two files exist separately. Neither suite could have caught the original defect, and arguably no unit test could: the bug was a denied IPC call whose only symptom lived in another process's window list. It was confirmed by reading the titles back out of the running app through the accessibility API (`System Events`) — `Märklig, Märklig, Märklig` before, and after, the three real ones. The folder-only window is the load-bearing case in that check: its session entry has `path: null`, so Rust alone could only have titled it `nerves_anbernic`, and observing `README.md — nerves_anbernic` is what proves the frontend's `setTitle` now lands too.
- **Exporting, printing or copying a `.typ` no longer produces a Markdown rendering of the Typst source** (#57). With a `.typ` open, all four file-out surfaces — Export → HTML, Export → PDF, Print, Copy as HTML — ran the Typst *source* through markdown-it and wrote the result into a file the dialog had already named `document.pdf`. `= Heading` came out as `<p>= Heading</p>`, `#set page(…)` and `#figure(…)` as paragraphs of prose, and `$x^2 + y^2$` was picked up as KaTeX by coincidence — while the correctly compiled pages sat in the preview pane at that same moment. Nothing was disabled and nothing warned. The cause was a caller, not a renderer: `buildHtmlExport` is the markdown-it pipeline and takes no format argument, and `buildCurrentHtml` called it unconditionally. `detectFormat` was consulted eleven times in `main.ts` — mode toggling, pane layout, zoom routing, session open, the `data-format` attribute — and not once between building the export HTML and writing it out. All four surfaces now go through `decideExportRoute` (`src/export/route.ts`), which returns `markdown`, `typst` with the compiled pages, or `blocked`; there is deliberately no fallback from `typst` to `markdown`, because that fallback *is* the bug. A `.typ` export wraps the SVG pages `typst_compile` already returns (`src/export/typst-html.ts`) — the same first-party render the pane is showing — so there is no new rendering backend and no new dependency: `typst-svg` is already built. `@page` is set from the page's own viewBox, so a PDF capture pages the document at its real size (A4 stays A4) instead of reflowing it onto US Letter. Exports force any pending debounced compile to finish first, so they can't silently omit the last keystroke, and refuse with a notice — rather than writing something — when the document has never compiled or currently has errors, since a document that doesn't compile has no faithful export. Pages go through `sanitizeSvg`, matching the preview pane and unlike the Mermaid export path, which injects raw because DOMPurify strips its `<foreignObject>` labels; `sanitizeSvg` was already tuned for typst's `<use xlink:href="#g…">` glyph references, and tests now pin that it also preserves embedded raster images, clip paths and glyph `<symbol>` definitions, so sanitizing costs nothing here. Neither baseline suite could see any of this: `cargo test` never touches the frontend, and vitest covers `buildHtmlExport` thoroughly but not its caller inside the `main.ts` bootstrap, which has no unit coverage at all. The gate is a new Playwright spec that drives the real menu-action listener and captures the exact string handed to each sink — `write_text_file`, `export_pdf`, `navigator.clipboard.write`, and the print iframe's own DOM — asserting each is the compiled document and carries no trace of markdown-it having seen the source. Five of its six cases fail on the unfixed code; the sixth is a Markdown control that passes on both, so correcting Typst cannot have been achieved by breaking Markdown. The related spec non-goal is amended in the same change: what stays deferred is a `typst-pdf` backend, not honest behavior from menu items that already shipped.
- **Save As on a `.typ` no longer converts the document to Markdown.** File → Save As offered `document.md` with a Markdown-only file filter for *every* document, so saving an open Typst file under a new name changed its identity — it reopened as Markdown with the source rendered as prose, one line away from the export defect above. Save As is a rename, not a conversion: the default name and the dialog filter now follow the open document's format (`saveMarkdownAs` becomes `saveDocumentAs(format, …)`, taking its extension lists from `src/format` instead of a second hardcoded copy). Saving a `.typ` elsewhere also re-opens the compiler session against the new path, so relative `#import "./sibling.typ"` and image references resolve against the file's new directory rather than the old one.
