...
- **Typst dependency watching** (`typst-dep-watcher.ts` + `commands/typst_dep_watcher.rs`) is a *second, separate* watcher, and the distinction matters: the document watcher above owns the open file, where an external change means the on-screen buffer is stale and something must be reconciled. A changed **import** is the opposite case — the buffer is still correct and must not be touched; only the render is stale, so the only correct response is `scheduleTypstCompile()`. Wiring imports into `installWatcher` would have inherited the reload/prompt/orphan behavior, which is wrong for every one of them. The path set is `CompileResult.dependencies`, which the compiler produces by recording what it *read* (`ViewerWorld::begin/finish_dependency_capture`), not by parsing import statements — so it covers transitive imports at any depth, plus `read()` and `image()` assets, and needs no parser of its own. `main.ts` filters the entry file out of the set before syncing, since the document watcher already covers it and feeding it to both compiles twice per save. Missing dependency parents use a recursive watch on the nearest existing ancestor; structural ancestor events invalidate the dependency too, covering mkdir/write races. Watch installation is transactional: an error retains the old watcher and rejects the sync so the frontend can retry, rather than caching an unwatched set as successful. A failed compile may only *add* to the watch set, never replace it: a parse error in the entry file never reaches the imports, so adopting that compile's near-empty set would unwatch the very file the user is about to fix. Same `EventTarget::Any` caveat as above — the frontend filters on "is this path in my set", which is also what makes two windows on one project both recompile.
