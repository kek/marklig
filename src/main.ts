...
async function bootstrap(): Promise<void> {
  ...
  if (isMainWindow()) {
    await buildAndAttachMenu({
      ...
      closeWindow: async () => {
        // Close acts on the window itself, not its state — call .close()
        // directly so its own onCloseRequested handler (with the dirty prompt
        // for its doc) fires. The companion "Cmd-W closes the focused window"
        // fix lives in its own PR; here we keep the trunk behaviour.
